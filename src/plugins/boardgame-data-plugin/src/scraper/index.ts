...
export const getBoardGameLinks = async (): Promise<BoardgameInfo[]> => {
	...
	try {
		...
		boardGameLinks = (await page.evaluate(() => {
			const wrapSelector = <T>(selector: string, doWork: (selector: string) => T | null) => {
				try {
			...
			return wrapSelector('#collectionitems > tbody > tr', (selector) => {
				...
				return testGames.map<BoardgameInfo>((game) => {
					const rank = parseInt(game.querySelector('.collection_rank > a')
						.getAttribute('name'))
					const gameDetails = game.querySelector('.collection_objectname')
					const objectId = parseInt(gameDetails.querySelector('.primary').getAttribute('href').match(/\/([0-9]*)\//)[1])
